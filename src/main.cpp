...
WiFiUDP nptUDP;
int timezoneOffset = 0 * 3600;
NTPClient timeclient(nptUDP, timezoneOffset);

float temperature, humidity, pressure, pressureAsl;

int altitude = 55;
String czas;

ESP8266WebServer server(137);
...
void setup()
{
  ...
  Serial.print("Connected to NTP server. TIME:");
...
void handleJson(String payload)
{
  ...
  temperature = doc["weather"]["temperature"];
  pressure = doc["weather"]["pressure"];
  pressureAsl = doc["weather"]["pressureASL"];
  humidity = doc["weather"]["humidity"];
  
  pressure = pressure / 100.0F;
  pressureAsl = pressureAsl / 100.0F;
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
  ...
  case WStype_TEXT:
    ...
    // if(payload[0] == '#')
    // {
    //   temperature = float(strtol((const char *) &payload[1], NULL, 10)) / 100;
    // }
...
//   ptr += "<meta charset=\"utf-8\" name='viewport' content='width=device-width, initial-scale=1.0'>";
...
//   ptr += "</div>";
//   ptr += "<div class='side-by-side text'>Wilgotność</div>";
//   ptr += "<div class='side-by-side reading'>";
//   ptr += (float)humidity;
...
//   ptr += "</div>";
//   ptr += "<div class='side-by-side text'>Ciśnienie</div>";
//   ptr += "<div class='side-by-side reading'>";
//   ptr += (float)pressure;
...
//   ptr += "</div>";
//   ptr += "<div class='side-by-side text'>Ciśnienie ASL</div>";
//   ptr += "<div class='side-by-side reading'>";
//   ptr += (float)pressureAsl;
//   ptr += "<span class='superscript'>hPa</span></div>";
//   ptr += "</div>";
//   ptr += "</div>";
//   ptr += "</body>";
